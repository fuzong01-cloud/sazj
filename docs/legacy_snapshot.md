...
本文档记录结构化重构前的仓库接手状态。本阶段目标是理解并保留当前基线，不删除重要文件，也不进行大范围重写。
...
- 项目仍是单体 Flask 页面应用，不是前后端分离平台。
- `app.py` 同时承担路由、模型加载、图片预处理、天气访问、大模型访问、兜底建议和聊天逻辑，职责过重。
- `templates/index.html` 包含大量内联样式和脚本，前端尚未按 Vue 或 MVVM 边界组织。
...
- 在 FastAPI 后端具备等价的健康检查、模型状态和预测接口前，保持 `app.py` 可运行。
- 将模型加载和预测逻辑移动到后端服务层。
...
python -c "import pathlib; compile(pathlib.Path('app.py').read_text(encoding='utf-8'), 'app.py', 'exec'); print('app.py compile ok')"
python -c "import pathlib; compile(pathlib.Path('webapp.py').read_text(encoding='utf-8'), 'webapp.py', 'exec'); print('webapp.py compile ok')"
python app.py
```
